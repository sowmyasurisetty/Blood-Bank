...
def adddonar():
    ...
    def add():
        ...
        if (phone.isdigit() == TRUE and len(phone) < 10):
            ...
            cur = conn.cursor()
            cur.execute("CREATE TABLE IF NOT EXISTS reco(name TEXT NOT NULL, age INT NOT NULL,sex TEXT NOT NULL,blood_group TEXT NOT NULL,address TEXT NOT NULL,contact_no TEXT NOT NULL,date DATE NOT NULL)")

            cur.execute("INSERT INTO reco (name, age, sex, blood_group, address, contact_no, date) VALUES (?, ?, ?, ?, ?, ?, ?)",
                        (e1.get(), e2.get(), e3.get(), e4.get(), e5.get('1.0', END), e6.get(), e7.get()))
...
def fetch1():
    conn = sqlite3.connect('blood_bank.db')
    cur = conn.cursor()
    cur.execute("SELECT blood_group, COUNT(*) AS count FROM reco GROUP BY blood_group")
...
def fetch3():
    def srch():
        conn = sqlite3.connect('blood_bank.db')
        cur = conn.cursor()
        cur.execute("SELECT * FROM reco WHERE blood_group=?", (e11.get(),))
...
b1 = Button(root, text="ADD DONAR", command=adddonar, width=30, bg="green", fg="white", relief=GROOVE,
            font=("times new roman", 10))
b1.place(x=450, y=100)

b2 = Button(root, text="SEARCH DONAR", width=30, bg="green", fg="white", relief=GROOVE, command=fetch3,
            font=("times new roman", 10))
b2.place(x=450, y=150)

b3 = Button(root, text="VIEW BLOOD STOCK", width=30, bg="green", fg="white", relief=GROOVE, command=fetch1,
            font=("times new roman", 10))
b3.place(x=450, y=200)

b4 = Button(root, text="VIEW DONAR LIST", width=30, bg="green", fg="white", relief=GROOVE, command=fetch,
            font=("times new roman", 10))
b4.place(x=450, y=250)

b5 = Button(root, text="EXIT", bg="green", width=30, command=root.destroy, fg="white", relief=GROOVE,
            font=("times new roman", 10))
